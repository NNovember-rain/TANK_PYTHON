...
while True:
    ...
    #nút về menu
    BACK_MENU_BUTTON=Button.Button(image_button,pos=(WIDTH_SCREEN/2,460),text_input="MENU",font=get_font(50),
                                base_color="#d7fcd4",hovering_color="yellow")
    
    #tạo một list chứa các đối tượng nút
    LIST_OPTION=list()
    LIST_OPTION.append(REPLAY_BUTTON)
    LIST_OPTION.append(BACK_MENU_BUTTON)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN: 
            if REPLAY_BUTTON.checkForInput(END_GET_POS):
                path = 'PvP.py'
                break
            if BACK_MENU_BUTTON.checkForInput(END_GET_POS):
                path = 'Main.py'
                break
    if path != '':
        break
